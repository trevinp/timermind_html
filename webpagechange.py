...
# url to be scraped
url = "https://sodastream.com/products/exchange-cylinder"
...
def getHash():
    ...
    textfile = open("textfile" + str(random.randint(0,5000)) + ".txt", "ab")
    textfile.write(the_page)
    textfile.close()
...
while 1: # Run forever
    ...
    if getHash() == current_hash: # If nothing has changed
        print (current_time, ": Not Changed")

    else: # If something has changed
        print ("Page Changed: " + url + " at " , current_time)
        break
